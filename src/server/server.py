            ...
            async for message in websocket:
                ...
                try:
                    ...
                    if msg_type == "input":
                        ...
                        success = self.logic.weapon_system.set_weapon(weapon)
                        if success:
                            self.logic.log(f"Player changed weapon to: {weapon}")
                        await websocket.send(
                            json.dumps(
                                {
                                    "type": "weapon_changed",
                                    "weapon": self.logic.weapon_system.current_weapon,
                                    "ammo": self.state.player.ammo,
                                }
                            )
                        )
                            ...
                            json.dumps(
                                {
                                    "type": "map_selected",
                                    "map": map_name,
                                    "map_name": self.state.get_map_info().get(
                                        "name", map_name
                                    ),
                            ...
                            json.dumps(
                                {
                                    ...
                                    "current_map_name": self.state.get_map_info().get(
                                        "name", "Unknown"
                                    ),
                                }
                            )
